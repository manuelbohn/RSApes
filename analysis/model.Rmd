...
```{r}
ggarrange(ggarrange(bp,cp, widths = c(1.5,1), labels = c("A","B")), cpRed, labels = c("","C"), heights = c(1.1,1), nrow = 2)
```
...
var gesMeaning = function(utterance, reaction){
  utterance.gesture == "pointLeft" ? flip(pointLeftMean) ? reaction.type == "pickLeft" : reaction.type == "pickRight":
  true
}
...
var literalListener = function(utterance){
  Infer({method: "enumerate", model: function(){
    ...
    var reaction = sample(Categorical({vs: all_reactions, ps: [0.5,0.5]}))
...
var utterancePrior = function(){
  var gesture = uniformDraw(["pointLeft", "pointRight"])
  return {gesture: gesture}
}


var speaker = function(reaction, speakerOptimality){
  Infer({method: "enumerate", model: function(){
...
var pragmaticListener = function(utterance){
  ...
  var S1 = speaker(reaction, speakerOptimality);
...
pragmaticListener({gesture: "pointLeft" })
...
literalListener({gesture: "pointLeft" })
...
pragPlot <- bind_rows(
  ...
  mutate(alpha = factor(alpha, levels = c("","1","5","10")))%>%
  filter(support == "pickLeft")
```
...
ggplot(pragPlot, aes(x = alpha, y = prob))+
  geom_bar(stat = "identity", aes(fill = support),position = position_dodge(), alpha = .5, col = "black")+
